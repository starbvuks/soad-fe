"use client";

import axios from "axios";
import React, { useState, useEffect } from "react";
import Link from "next/link";
import { useRouter } from "next/router";

export default function Page() {
  const [specializations, setSpecializations] = useState([]);
  const router = useRouter();

  const token =
    "";

  useEffect(() => {
    const fetchData = async () => {
      const res = await axios.get("http://localhost:1338/api/specializations", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setSpecializations(res.data.data);
    };

    fetchData();
  }, []); // Empty dependency array

  return (
    <div className="bg-white flex flex-col justify-center xl:py-12">
      <span className="text-center text-5xl font-bold">
        School of Art & Design
      </span>
      <span className="text-center text-4xl font-medium">
        Digital design archive
      </span>
      <div className="grid grid-cols-5 text-center gap-10 self-center xl:px-12 xl:text-2xl xl:mt-48">
        {specializations.map((spec, index) => (
          <Link
            href={{
              pathname: `/ay`,
              query: { specId: spec.id },
            }}
          >
            <div className="bg-slate-500 flex justify-center items-center px-8 py-32 h-full rounded-3xl">
              <span className="text-white font-semibold">
                {spec.attributes.specializationName}
              </span>
            </div>
          </Link>
        ))}
      </div>
    </div>
  );
}
